...
void Server(const FunctionCallbackInfo<Value>& args)
{
    if (args.IsConstructCall()) {
        ...
        Error serverError;
        uWS::Server* server = new uWS::Server(*es,
                                              &serverError,
                                              args[0]->IntegerValue(),
                                              args[1]->IntegerValue(),
                                              args[2]->IntegerValue());
        if (serverError == ERR_NONE) {
            ...
            args.This()->SetAlignedPointerInInternalField(CONNECTION_CALLBACK,
                                                          new Persistent<Function>);
            args.This()->SetAlignedPointerInInternalField(DISCONNECTION_CALLBACK,
                                                          new Persistent<Function>);
            args.This()->SetAlignedPointerInInternalField(MESSAGE_CALLBACK,
                                                          new Persistent<Function>);
            ...
            args.This()->Set(String::NewFromUtf8(args.GetIsolate(), "error"),
                             Boolean::New(args.GetIsolate(), true));
        }
...
void onConnection(const FunctionCallbackInfo<Value>& args)
{
    ...
    Persistent<Function>* connectionCallback =
      (Persistent<Function>*)args.Holder()->GetAlignedPointerFromInternalField(CONNECTION_CALLBACK);
...
{
    return socket.getData() ? Local<Value>::New(isolate, *(Persistent<Value>*)socket.getData())
...
void onMessage(const FunctionCallbackInfo<Value>& args)
{
    uWS::Server* server = (uWS::Server*)args.Holder()->GetAlignedPointerFromInternalField(0);
    Isolate* isolate = args.GetIsolate();
    Persistent<Function>* messageCallback =
      (Persistent<Function>*)args.Holder()->GetAlignedPointerFromInternalField(MESSAGE_CALLBACK);
    messageCallback->Reset(isolate, Local<Function>::Cast(args[0]));
    server->onMessage([isolate, messageCallback](uWS::WebSocket socket,
                                                 const char* message,
                                                 size_t length,
                                                 uWS::OpCode opCode) {
        HandleScope hs(isolate);
        Local<Value> argv[] = {
          wrapSocket(socket, isolate),
          node::Buffer::New(isolate, (char*)message, length, [](char* data, void* hint) {}, nullptr)
            .ToLocalChecked(),
...
void onPing(const FunctionCallbackInfo<Value>& args)
{
    uWS::Server* server = (uWS::Server*)args.Holder()->GetAlignedPointerFromInternalField(0);
    Isolate* isolate = args.GetIsolate();
    Persistent<Function>* pingCallback =
      (Persistent<Function>*)args.Holder()->GetAlignedPointerFromInternalField(PING_CALLBACK);
    pingCallback->Reset(isolate, Local<Function>::Cast(args[0]));
    server->onPing([isolate, pingCallback](
                     uWS::WebSocket socket, const char* message, size_t length) {
        HandleScope hs(isolate);
        Local<Value> argv[] = {
          wrapSocket(socket, isolate),
          node::Buffer::New(isolate, (char*)message, length, [](char* data, void* hint) {}, nullptr)
...
void onPong(const FunctionCallbackInfo<Value>& args)
{
    uWS::Server* server = (uWS::Server*)args.Holder()->GetAlignedPointerFromInternalField(0);
    Isolate* isolate = args.GetIsolate();
    Persistent<Function>* pongCallback =
      (Persistent<Function>*)args.Holder()->GetAlignedPointerFromInternalField(PONG_CALLBACK);
    pongCallback->Reset(isolate, Local<Function>::Cast(args[0]));
    server->onPong([isolate, pongCallback](
                     uWS::WebSocket socket, const char* message, size_t length) {
        HandleScope hs(isolate);
        Local<Value> argv[] = {
          wrapSocket(socket, isolate),
...
void onDisconnection(const FunctionCallbackInfo<Value>& args)
{
    ...
    Persistent<Function>* disconnectionCallback =
      (Persistent<Function>*)args.Holder()->GetAlignedPointerFromInternalField(
        DISCONNECTION_CALLBACK);
    disconnectionCallback->Reset(isolate, Local<Function>::Cast(args[0]));
    server->onDisconnection([isolate, disconnectionCallback](
...
void transfer(const FunctionCallbackInfo<Value>& args)
{
    ...
    if (args[0]->IsObject()) {
        uv_fileno(getTcpHandle(args[0]->ToObject()->GetAlignedPointerFromInternalField(0)),
                  (uv_os_fd_t*)fd);
    }
    else {
...
void sendCallback(uWS::WebSocket webSocket, void* data, bool cancelled)
{
    ...
    if (!cancelled) {
        ...
        node::MakeCallback(sc->isolate,
                           sc->isolate->GetCurrentContext()->Global(),
...
void prepareMessage(const FunctionCallbackInfo<Value>& args)
{
    ...
    NativeString nativeString(args[0]);
    Local<Object> preparedMessage =
      Local<Object>::New(args.GetIsolate(), persistentTicket)->Clone();
    preparedMessage->SetAlignedPointerInInternalField(
      0,
      WebSocket::prepareMessage(nativeString.getData(), nativeString.getLength(), opCode, false));
...
{
    WebSocket::finalizeMessage(
      (WebSocket::PreparedMessage*)args[0]->ToObject()->GetAlignedPointerFromInternalField(0));
}
